    ...
    public class LACK2BLL : ILACK2BLL
    {
        ...
        private void SubmitDocument(Lack2WorkflowDocumentInput input)
        {
            if (input.DocumentId != null)
            {
                ...
                var isDelegated = _poaDelegationServices.IsDelegatedUserByUserAndDate(dbData.CREATED_BY, input.UserId,
                    DateTime.Now);
        ...
        private string CommentDelegateUser(LACK2 dbData, Lack2WorkflowDocumentInput input)
        {
            string comment = "";

            var inputHistory = new GetByFormTypeAndFormIdInput();
            inputHistory.FormId = dbData.LACK2_ID;
            inputHistory.FormType = Enums.FormType.LACK2;
            ...
            else
            {
                ...
                {
                    listPoa = _poaBll.GetPoaActiveByPlantId(dbData.LEVEL_PLANT_ID).Select(c => c.POA_ID).ToList();
                }
        ...
        private MailNotification ProsesMailNotificationBody(Lack2Dto lackData, Lack2WorkflowDocumentInput input)
        {
            ...
            bodyMail.AppendLine();
            bodyMail.Append("<tr><td>Company Code </td><td>: " + lackData.Burks + "</td></tr>");
            bodyMail.AppendLine();
            bodyMail.Append("<tr><td>Plant ID </td><td>: " + lackData.LevelPlantId + " - " + lackData.LevelPlantName + "</td></tr>");
            bodyMail.AppendLine();
            bodyMail.Append("<tr><td>Period </td><td>: " + _monthBll.GetMonth(lackData.PeriodMonth).MONTH_NAME_ENG + "</td></tr>");
            bodyMail.AppendLine();
            bodyMail.Append("<tr><td>KPPBC </td><td>: " + _lfaBll.GetById(_nppbkcService.GetById(lackData.NppbkcId).KPPBC_ID).NAME2 + "</td></tr>");
            bodyMail.AppendLine();
            ...
            bodyMail.Append("<tr colspan='2'><td><i>To VIEW, Please click this <a href='" + webRootUrl + "/Lack2/Details/" + lackData.Lack2Id + "'>link</a></i></td></tr>");
            bodyMail.AppendLine();
            bodyMail.Append("<tr colspan='2'><td><i>To APPROVE, Please click this <a href='" + webRootUrl + "/Lack2/Edit/" + lackData.Lack2Id + "'>link</a></i></td></tr>");
            bodyMail.AppendLine();
            bodyMail.Append("</table>");
            bodyMail.AppendLine();
            ...
            switch (input.ActionType)
            {
                ...
                case Enums.ActionType.Approve:
                    ...
                    //first code when manager exists
                    else if (lackData.Status == Enums.DocumentStatus.WaitingForApprovalController)
                    {
                        foreach (var item in controllerList)
                        {
            ...
            inputDelegate.FormType = Enums.FormType.LACK2;
            inputDelegate.FormId = lackData.Lack2Id;
            inputDelegate.FormNumber = lackData.Lack2Number;
            inputDelegate.ActionType = input.ActionType;

            inputDelegate.CurrentUser = input.UserId;
            inputDelegate.CreatedUser = lackData.CreatedBy;
