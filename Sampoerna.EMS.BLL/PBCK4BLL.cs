...
namespace Sampoerna.EMS.BLL
{
    public class PBCK4BLL : IPBCK4BLL
    {
        ...
        public List<Pbck4Dto> GetPbck4ByParam(Pbck4GetByParamInput input)
        {
            ...
            {
               queryFilter = queryFilter.And(c => c.STATUS == Enums.DocumentStatus.Completed || c.STATUS == Enums.DocumentStatus.Cancelled || c.STATUS == Enums.DocumentStatus.GovRejected);
            }
            else
            {
               queryFilter = queryFilter.And(c => !(c.STATUS == Enums.DocumentStatus.Completed || c.STATUS == Enums.DocumentStatus.Cancelled || c.STATUS == Enums.DocumentStatus.GovRejected));
        ...
        public Pbck4Dto SavePbck4(Pbck4SaveInput input)
        {
            ...
            if (input.Pbck4Dto.PBCK4_ID > 0)
            {
                ...
                //{
                //    // need to check again ?
                //    if (_poaDelegationServices.IsDelegatedUserByUserAndDate(dbData.CREATED_BY, input.UserId,
                //        DateTime.Now))
                //    {
                //        inputWorkflowHistory.Comment = Constans.LabelDelegatedBy + dbData.CREATED_BY;
                //        //add to poa delegate history
                //    }
                //    else 
            ...
            switch (input.ActionType)
            {
                case Enums.ActionType.Submit:
                    if (pbck4Dto.Status == Enums.DocumentStatus.WaitingForApproval)
                    {
                        if (rejected != null)
                        {
                            rc.To.Add(_poaBll.GetActivePoaById(rejected.ACTION_BY).POA_EMAIL);
                        }
                        else
                        {
                ...
                case Enums.ActionType.Approve:
                    ...
                    {
                        var poaData = _poaBll.GetActivePoaById(pbck4Dto.CREATED_BY);
                        if (poaData != null)
                        {
                            //creator is poa user
                ...
                case Enums.ActionType.Reject:
                    ...
                    var poaData2 = _poaBll.GetActivePoaById(pbck4Dto.CREATED_BY);
                ...
                case Enums.ActionType.GovPartialApprove:
                    if (pbck4Dto.Status == Enums.DocumentStatus.Completed)
                    {
                        ...
                        var userData = _userBll.GetUserById(pbck4Dto.CREATED_BY);
                        rc.To.Add(userData.EMAIL);
                        var poaData3 = _poaBll.GetActivePoaById(pbck4Dto.CREATED_BY);
        ...
        public List<GetListBrandByPlantOutput> GetListFaCodeHaveBlockStockByNppbkc(string nppbkcId, List<string> plantList)
        {
            ...
            {
                var dbBrand = _brandRegistrationServices.GetBrandByPlant(plantId).ToList();
                ...
                foreach (var zaidmExBrand in dbBrand)
                {
                    ...
                    var blockStock = GetBlockedStockQuota(plantId, zaidmExBrand.FA_CODE,zaidmExBrand.STICKER_CODE);
        ...
        public GetBrandItemsOutput GetBrandItemsStickerCodeByNppbkcAndFaCode(string nppbkc, string faCode, List<string> plantList,string stickerCode = null)
        {
            var dbBrand = new ZAIDM_EX_BRAND();
            ...
            foreach (var plantId in plantList)
            {
                ...
                {
                    dbBrand = _brandRegistrationServices.GetByPlantIdAndFaCode(plantId, faCode);
                }
        ...
        public BlockedStockQuotaOutput GetBlockedStockQuota(string plant, string faCode,string stickerCode = null)
        {
            var dbBrand = _brandRegistrationServices.GetByPlantIdAndFaCode(plant, faCode);
            if (stickerCode != null)
            {
               dbBrand = _brandRegistrationServices.GetByPlantIdAndFaCodeStickerCode(plant, faCode, stickerCode);
            }
        ...
        public decimal GetCurrentReqQtyByPbck4IdAndFaCode(int pbck4Id, string faCode, string stickerCode = null)
        {
            decimal result = 0;
            ...
            if (dbPbck != null)
            {
                ...
                {
                    if (pbck4Item.FA_CODE == faCode && pbck4Item.STICKER_CODE == stickerCode)
                    {
        ...
        public List<GetListCk1ByNppbkcOutput> GetListCk1ByPlantAndFaCode(GetListCk1ByPlantAndFaCodeInput input)
        {
            ...
            foreach (var ck1 in dbCk1)
            {
                foreach (var ck1Item in ck1.CK1_ITEM)
                {
                    ...
                    if (input.StickerCode != null)
                    {
                        if (ck1Item.FA_CODE == input.FaCode && ck1Item.MATERIAL_ID == input.StickerCode)
                        {
        ...
        public List<Pbck4Dto> GetAllByParam(Pbck4DasboardParamInput input)
        {
            ...
            {
                queryFilter = queryFilter.And(c => c.REPORTED_ON.HasValue && c.REPORTED_ON.Value.Month == input.Month.Value);
            }
            if (input.Year.HasValue)
            {
                queryFilter = queryFilter.And(c => c.REPORTED_ON.HasValue && c.REPORTED_ON.Value.Year == input.Year.Value);
            }
            if (!string.IsNullOrEmpty(input.Creator))
            {
                queryFilter = queryFilter.And(c => c.CREATED_BY == input.Creator);
            ...
            Func<IQueryable<PBCK4>, IOrderedQueryable<PBCK4>> orderBy = null;
            {
                ...
                {
                    orderBy = c => c.OrderBy(OrderByHelper.GetOrderByFunction<PBCK4>(input.ShortOrderColumn));
                }
                ...
                var mapResult = Mapper.Map<List<Pbck4Dto>>(dbData.ToList());
