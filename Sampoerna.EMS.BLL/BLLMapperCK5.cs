    ...
    public partial class BLLMapper
    {
        public static void InitializeCK5()
        {
            ...
            Mapper.CreateMap<CK5, CK5ReportDetailsDto>().IgnoreAllNonExisting()
                ...
                .ForMember(dest => dest.FacilityNumber, opt => opt.MapFrom(src => src.INVOICE_NUMBER))
                .ForMember(dest => dest.FacilityDate, opt => opt.MapFrom(src => src.INVOICE_DATE))
            ...
            Mapper.CreateMap<CK5_MATERIAL, CK5ReportMaterialDto>().IgnoreAllNonExisting()
                .ForMember(dest => dest.Total, opt => opt.MapFrom(src => src.QTY.HasValue ? src.QTY.Value.ToString() : "0"))
                .ForMember(dest => dest.Uom, opt => opt.MapFrom(src => src.UOM))
                .ForMember(dest => dest.TotalConverted, opt => opt.MapFrom(src => src.CONVERTED_QTY.HasValue ? src.CONVERTED_QTY.Value.ToString() : "0"))
                ...
                .ForMember(dest => dest.ConvertionItem, opt => opt.MapFrom(src => src.CONVERTION.HasValue ? src.CONVERTION.Value.ToString() : "0"))
                .ForMember(dest => dest.ConvertionUomItem, opt => opt.MapFrom(src => src.CONVERTED_UOM))
