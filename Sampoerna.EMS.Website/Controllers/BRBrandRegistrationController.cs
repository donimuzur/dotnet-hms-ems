    ...
    public class BRBrandRegistrationController : BaseController
    {
        ...
        public BrandAccess GetUserAccess(long BrandRegId, long intLastApprovedStatus, string CreatedBy, string LastApprovedBy, string Action, string NPPBKCId = "")
        {
            ...
            if (intLastApprovedStatus == 0)
            {
                ...
                {
                    case Sampoerna.EMS.Core.Enums.UserRole.Administrator:
                        result.CanCreate = true;
                        break;

                    case Sampoerna.EMS.Core.Enums.UserRole.POA:
                        result.CanCreate = true;
                        break;
            ...
            else
            {
                ...
                if (CurrentUser.UserRole == Sampoerna.EMS.Core.Enums.UserRole.Viewer)
                {
                    result.CanView = true;
                }
                else
        ...
        private BrandRegistrationReqViewModel GeneratePropertiesBrand(BrandRegistrationReqViewModel source, bool update)
        {
            ...
            IEnumerable<Sampoerna.EMS.Core.Enums.DocumentStatusGovType2> statusTypes = Enum.GetValues(typeof(Sampoerna.EMS.Core.Enums.DocumentStatusGovType2)).Cast<Sampoerna.EMS.Core.Enums.DocumentStatusGovType2>();
            data.ListGovStatus = from form in statusTypes
                                 select new SelectListItem
                                 {
                                     Text = EnumHelper.GetDescription((Enum)Enum.Parse(typeof(Sampoerna.EMS.Core.Enums.DocumentStatusGovType2), form.ToString())),
        ...
        public ActionResult IndexBrandRegistration()
        {
            ...
            try
            {
                ...
                if (CurrentUser.UserRole == Enums.UserRole.Administrator || CurrentUser.UserRole == Enums.UserRole.POA || CurrentUser.UserRole == Enums.UserRole.Viewer)
                {
                    ...
                    IEnumerable<Sampoerna.EMS.Core.Enums.BrandRegistrationAction> actionTypes = Enum.GetValues(typeof(Sampoerna.EMS.Core.Enums.BrandRegistrationAction)).Cast<Sampoerna.EMS.Core.Enums.BrandRegistrationAction>();
                    model.SearchInput.ListRegistrationType = from form in actionTypes
                                                             select new SelectListItem
                                                             {
                                                                 Text = EnumHelper.GetDescription((Enum)Enum.Parse(typeof(Sampoerna.EMS.Core.Enums.BrandRegistrationAction), form.ToString())),
                                                                 Value = ((int)form).ToString()
                                                             };
        ...
        [HttpPost]
        public ActionResult RestorePrintoutToDefault(int RegistrationType, string CreatedBy, bool DocExport)
        {
            if (RegistrationType == 1)
            {
                if (DocExport)
                {
                    var ErrMessage = refService.RestorePrintoutToDefault("BRAND_REGISTRATION_EXPORT", CreatedBy);
                    return Json(ErrMessage);

                }
                else
                {
                    var ErrMessage = refService.RestorePrintoutToDefault("BRAND_REGISTRATION_NEWBRAND", CreatedBy);
                    return Json(ErrMessage);
                }
            }
            else
            {
                var ErrMessage = refService.RestorePrintoutToDefault("BRAND_REGISTRATION_UPDATEHJE", CreatedBy);
                return Json(ErrMessage);
            }
        ...
        private string GetPrintout(BrandRegistrationReqViewModel _BRViewModel, long ID)
        {
            ...
            parameters.Add("LAMPIRAN_COUNT", Convert.ToString(_BRViewModel.Count_Lamp));
        ...
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult UpdatePrintOutLayout(string NewPrintout, int RegistrationType, string CreatedBy, bool DocExport)
        {
